# This program is a chess game. The user PLays with the
# computer.
#
...
import threading #To allow for System to think simultaneously while the GUI is coloring the board.
...
# 3. System related functions - These are the functions involved in helping the
...
# best move to be played for the System. Alpha and beta are lower and upper bounds to
...
# to be played by the System. It also checks the opening table to see if there is a 
...
def chess_coord_to_pixels(chess_coord):
    ...
    if isSystem:
        if SysPlayer==0:
            #This means you're playing against the System and are playing as black:
...
def drawBoard():
    ...
    if isDraw or chessEnded or isSystemThink:
        #Shades
    ...
    if not (isDraw or chessEnded or isSystemThink):
        for shade in listofShades:
            img,chess_coord = shade.getInfo()
            pixel_coord = chess_coord_to_pixels(chess_coord)
            screen.blit(img,pixel_coord)
...
def evaluate(position):
    ...
    #to count pieces were attempts at making the System more efficient.
    ...
    #to make the System smarter when analysing boards and has not been 
...
#this value.
isSystemThink = False #Stores whether or not the System is calculating the best move to be played.
...
#For animating System thinking graphics:
ax,ay=0,0
numm = 0
...
while not gameEnded:
    if isMenu:
        ...
        if isSystem==True:
            #The user has selected to play against the System.
        ...
        if isFlip!=-1:
            ...
            #play as black, call upon the System to make a move:
            if isSystem and SysPlayer==0:
                colorsign=1
                bestMoveReturn = []
    ...
    if isSystemThink and numm%6==0:
        ...
        if ax%4==0:
            createShades([])
        #If the System is white, start from the opposite side (since the board is flipped)
        if SysPlayer==0:
    ...
    for event in pygame.event.get():
        ...
        #completely ignored:
        if chessEnded or isTransition or isSystemThink:
            continue
        ...
        if (isDown or isClicked) and event.type == MOUSEBUTTONUP:
            ...
            if isCheckmate(position,'black'):
                winner = 'w'
                chessEnded = True
            #If the System option was selecteed and the game still hasn't finished,
            #let the System start thinking about its next move:
            if isSystem and not chessEnded:
                if player==0:
                    colorsign = 1
                else:
                    colorsign = -1
    ...
    #If the System is thinking, make sure to check if it isn't done thinking yet.
    #Also, if a piece is currently being animated don't ask the System if it's
    #done thining, in case it replied in the affirmative and starts moving 
    #at the same time as your piece is moving:
    if isSystemThink and not isTransition:
